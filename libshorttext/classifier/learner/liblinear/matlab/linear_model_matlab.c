...
const char *matlab_matrix_to_model(struct model *model_, const mxArray *matlab_struct)
{
	...
	// Parameters
	ptr = mxGetPr(rhs[id]);
	model_->param.solver_type = (INT64)ptr[0];
	id++;

	// nr_class
	ptr = mxGetPr(rhs[id]);
	model_->nr_class = (INT64)ptr[0];
	id++;
	...
	// nr_feature
	ptr = mxGetPr(rhs[id]);
	model_->nr_feature = (INT64)ptr[0];
	id++;

	// bias
	ptr = mxGetPr(rhs[id]);
	model_->bias = (INT64)ptr[0];
	id++;
	...
	{
		model_->label = Malloc(INT64, model_->nr_class);
		ptr = mxGetPr(rhs[id]);
		for(i=0;i<model_->nr_class;i++)
			model_->label[i] = (INT64)ptr[i];
	}
	id++;
